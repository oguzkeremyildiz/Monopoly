...
public class PlayerTypeTwo extends Player{
    ...
    public PlayerTypeTwo(String name, int place, int money, Properties properties, boolean jail, Random dice){
                ...
                for (int j = 0; j < allProperties.size(); j++) {
                    if (allProperties.getProperty(j).getTableNo() == getPlace()) {
                        if (!allProperties.getProperty(j).isStatus()){
                            if (getMoney() - allProperties.getProperty(j).getPrice() >= 30){
